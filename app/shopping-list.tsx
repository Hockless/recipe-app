...
export default function ShoppingListScreen() {
  ...
  const [seededAliases, setSeededAliases] = useState<
  ...
  const stripParentheticals = (s: string) =>
    s
      .replace(/\([^)]*\)/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  ...
  const buildAliasMap = () => {
    ...
    // Built-ins removed (Ingredient database lived in Kitchen & Pantry feature)
    // Seeded
  ...
  const canonicalizeName = (name: string) => {
    ...
    if (
      /^garlic\b.*\bcloves?\b/.test(k) ||
      (/^garlic\b/.test(k) && /\bcloves?\b/.test(k))
    )
      return 'Garlic';
    if (/^spring onion/.test(k) || /^spring\s*onions?$/.test(k))
      return 'Spring onion';
    if (
      /^onions?$/.test(k) ||
      (/\bonion\b/.test(k) && !/spring\s*onion/.test(k))
    )
      return 'Onion';
  ...
  const loadData = async () => {
    try {
      ...
      // Load checked items
      const storedCheckedItems = await AsyncStorage.getItem(
        'shoppingListChecked',
      );
      ...
      // Load custom items
      const storedCustomItems = await AsyncStorage.getItem(
        'customShoppingItems',
      );
  ...
  const generateShoppingList = () => {
    ...
    // Kitchen & Pantry removed (no fridge skip logic)
    const fridgeSet = new Set<string>();
    const pantryMap = new Map<string, { qty: number; unit: string }>();
    pantryItems.forEach(p =>
      pantryMap.set(p.name.toLowerCase() + '::' + p.unit, {
        qty: p.quantity,
        unit: p.unit,
      }),
    );
    const requiredAgg = new Map<
      string,
        ...
        if (recipe) {
          recipe.ingredients.forEach(ingredient => {
            ...
            const expandAlternatives = (raw: string): string[] => {
              ...
              if (/rosemary\s+sprig\s+or\s+dried\s+rosemary/.test(lowered))
                return ['Rosemary'];
            ...
            nameVariants.forEach((variant, idx) => {
              ...
              if (qty && unit) {
                ...
                if (ingredientMap[canonicalName]) {
                  const amt =
                    workingIng.amount + (isAlternative ? ' (alt)' : '');
                  if (
                    workingIng.amount &&
                    !ingredientMap[canonicalName].amounts.includes(amt)
                  ) {
                    ingredientMap[canonicalName].amounts.push(amt);
                  }
                  if (
                    !ingredientMap[canonicalName].recipes.includes(recipe.title)
                  ) {
    ...
    for (const req of requiredAgg.values()) {
      ...
      if (deficit > 0.0001) {
        ...
        if (ingredientMap[req.name]) {
          if (!ingredientMap[req.name].amounts.includes(amountStr))
            ingredientMap[req.name].amounts.push(amountStr);
          if (!ingredientMap[req.name].recipes.includes('Multiple'))
            ingredientMap[req.name].recipes.push('Multiple');
        } else {
  ...
  const getFutureMealInfo = () => {
    if (mealPlan.length === 0)
      return { dateRange: '', futureMealCount: 0, totalMealCount: 0 };
    ...
    if (futureMeals.length === 0) {
      return {
        dateRange: 'No upcoming meals',
        futureMealCount: 0,
        totalMealCount: mealPlan.length,
      };
    }

    const futureDates = futureMeals
      .map(meal => new Date(meal.date))
  ...
  const addCustomItem = async () => {
    ...
    const amountStr =
      customItemAmount.trim() ||
      buildAmount(customQty, customUnit, customCustomUnit);
  ...
  const saveToHistory = async () => {
    ...
    const existingIndex = shoppingHistory.findIndex(
      h => h.weekStart === weekStart && h.weekEnd === weekEnd,
    );

    const historyItem: ShoppingListHistory = {
      id:
        existingIndex >= 0
          ? shoppingHistory[existingIndex].id
          : Date.now().toString(),
      weekStart,
      weekEnd,
      items: [...shoppingList],
  ...
  const checkedCount = shoppingList.filter(
    item => checkedItems[item.name.toLowerCase().trim()],
  ).length;
  // (no fridge integration)
  ...
  const categorizeItem = (rawName: string): string => {
    ...
    if (/\bwhite pepper\b|\bblack pepper\b/.test(name))
      return 'Herbs, Spices & Seasoning';
    if (/peppercorns?/.test(name)) return 'Herbs, Spices & Seasoning';
    if (/(^|\b)(dried\s*)?chill?i( |-)?flakes?\b/.test(name))
      return 'Herbs, Spices & Seasoning';
    ...
    if (
      /\bpepper\b/.test(name) &&
      !/(bell pepper|red pepper|green pepper|yellow pepper)/.test(name) &&
      /\b(black|white|pink|cayenne|aleppo|sichuan)\b/.test(name)
    ) {
      return 'Herbs, Spices & Seasoning';
    }
  ...
  const grouped = visibleList.reduce<Record<string, typeof shoppingList>>(
    (acc, item) => {
  ...
  const hoistedOrder =
    hoistCategory &&
    hoistCategory !== 'None' &&
    baseCategoryOrder.includes(hoistCategory)
      ...
      <ThemedView style={styles.header}>
        ...
        <ThemedText style={styles.subtitle}>
          {currentHistoryIndex >= 0
            ...
            : shoppingList.length > 0
              ? `${checkedCount}/${shoppingList.length} items completed`
        ...
        {mealPlan.length > 0 && currentHistoryIndex === -1 && (
          <ThemedView style={styles.weekInfo}>
            ...
            <ThemedText style={styles.mealCount}>
              {getFutureMealInfo().futureMealCount} future meal
              {getFutureMealInfo().futureMealCount !== 1 ? 's' : ''} planned
              {getFutureMealInfo().totalMealCount >
                getFutureMealInfo().futureMealCount &&
        ...
        {shoppingList.length === 0 ? (
          <ThemedView style={styles.emptyState}>
            ...
            <ThemedText style={styles.emptyDescription}>
              Your shopping list will be automatically generated from your
              weekly meal plan.
            </ThemedText>
            <TouchableOpacity
            ...
            >
              <ThemedText style={styles.planMealsButtonText}>
                Plan Your Meals
              </ThemedText>
            </TouchableOpacity>
          </ThemedView>
          ...
          <>
            {checkedCount > 0 && (
              ...
              >
                <ThemedText style={styles.clearButtonText}>
                  Clear {checkedCount} completed item
                  {checkedCount !== 1 ? 's' : ''}
                </ThemedText>
              </TouchableOpacity>
            )}
            ...
            <ThemedView style={styles.hoistRow}>
              <ThemedText style={styles.hoistLabel}>
                Ingredient type:
              </ThemedText>
              <TouchableOpacity
                style={styles.hoistSelect}
                onPress={() => setShowHoistPicker(true)}
              >
                <ThemedText style={styles.hoistSelectText}>
                  {hoistCategory === 'None' ? 'None' : hoistCategory}
                </ThemedText>
              </TouchableOpacity>
              {hoistCategory !== 'None' && (
                <TouchableOpacity
                  style={styles.hoistClear}
                  onPress={() => setHoistCategory('None')}
                >
            ...
            <ThemedView style={styles.filterRow}>
              <TouchableOpacity
                style={[
                  styles.filterToggle,
                  showUncheckedOnly && styles.filterToggleActive,
                ]}
              ...
              >
                <ThemedText
                  style={[
                    ...
                    showUncheckedOnly && styles.filterToggleTextActive,
                  ]}
                >
                  {showUncheckedOnly
              ...
              {showUncheckedOnly && (
                <ThemedText style={styles.filterMeta}>
                  {visibleList.length} left •{' '}
                  {shoppingList.length - visibleList.length} hidden
                </ThemedText>
              )}
            </ThemedView>
                ...
                <View style={styles.modalCard}>
                  ...
                  <ScrollView style={{ maxHeight: 320 }}>
                    {(['None', ...baseCategoryOrder] as string[]).map(name => (
                      <TouchableOpacity
                        key={name}
                        style={styles.hoistOption}
                        onPress={() => {
                          setHoistCategory(name as any);
                          setShowHoistPicker(false);
                        }}
                      >
                        <ThemedText style={styles.hoistOptionText}>
                          {name}
                        </ThemedText>
                      </TouchableOpacity>
                    ))}
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => setShowHoistPicker(false)}
                  >
              ...
              {hoistedOrder
                ...
                .map(category => (
                  <ThemedView key={category} style={styles.categoryBlock}>
                    <ThemedText style={styles.categoryHeading}>
                      {category}
                    </ThemedText>
                ...
                <View style={styles.modalCard}>
                  ...
                  <ScrollView style={{ maxHeight: 300 }}>
                    {(
                      ...
                      <View key={group}>
                        <ThemedText style={styles.unitGroupLabel}>
                          {group}
                        </ThemedText>
                        <View
                          style={{ flexDirection: 'row', flexWrap: 'wrap' }}
                        >
                          {UNIT_OPTIONS.filter(u => u.group === group).map(
                            (u: UnitOption) => (
                              <TouchableOpacity
                                key={u.value}
            ...
            {shoppingHistory.length === 0 ? (
              ...
              shoppingHistory.map((history, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.historyItem,
                    currentHistoryIndex === index && styles.activeHistoryItem,
                  ]}
                ...
                >
                  <ThemedView style={styles.historyItemContent}>
                    <ThemedText style={styles.historyWeek}>
                      📅{' '}
                      {new Date(history.weekStart).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                      })}{' '}
                      -{' '}
                      {new Date(history.weekEnd).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </ThemedText>
                    <ThemedText style={styles.historyDetails}>
                      ...
                      {history.mealPlanSnapshot.length} meals planned
                    </ThemedText>
                    <ThemedText style={styles.historyDate}>
                      Saved:{' '}
                      {new Date(history.dateCreated).toLocaleDateString()}
                    </ThemedText>
                  </ThemedView>
